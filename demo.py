...
if cases == 'India cases':
    ...
    df = pd.concat([df['sno'],df['state_name'], df['active'], df['cured'], df['death']],axis=1) 
    ...
    df['lat']=[15.9129,11.7401,28.2180,26.244156,25.0961,30.7333,21.295132,20.1809,28.7041,15.2993,22.309425,29.065773,32.084206,33.7782,23.6102,15.317277,10.850516,34.209515,23.473324,19.663280,24.6637,25.4670,23.1645,20.940920,11.9416,31.1471,27.0238,11.1271,18.1124,23.9408,30.0668,26.8467,22.9868]
    df['lon']=[79.7400,92.6586,94.7278,92.537842,85.3131,76.7794,81.828232,73.0169,77.1025,74.1240,72.136230,76.040497,77.571167,76.5762,85.2799,75.713890,76.271080,77.615112,77.947998,75.300293,93.9063,91.3662,92.9376,84.803467,79.8083,75.3412,74.2179,78.6569,79.0193,91.9882,79.0193,80.9462,87.8550]
    fig = px.scatter_mapbox(df,lat="lat", lon="lon", hover_name="state_name", hover_data=["active",'cured','death'],
                            color_discrete_sequence=["firebrick"], zoom=3, height=300)
    fig.update_layout(mapbox_style="open-street-map")
    fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
    st.plotly_chart(fig)
    ...
    def bar_graph():
        ...
        plt.barh(df['state_name'],df['active'] ,color=['red','blue','green','orange','yellow','pink'])
        for index, value in enumerate(df['active']):
            plt.text(value, index, str(value))
    ...
    def pie_chart():
        ...
        size = [sum(df['active']),sum(df['cured']),sum(df['death'])]
        labels = ['Confirmed\n'+str(sum(df['active'])),
                'Recovered\n'+str(sum(df['cured'])),
                'Deceased\n'+str(sum(df['death']))]
    ...
    st.sidebar.subheader('Confirmed')
    st.sidebar.info(str(sum(df['active'])))
    st.sidebar.subheader('Recovered')
    st.sidebar.success(str(sum(df['cured'])))
    st.sidebar.subheader('Deaths')
    st.sidebar.warning(str(sum(df['death'])))
